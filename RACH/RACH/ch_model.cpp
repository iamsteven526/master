...
void MultipleAccessChannel(double stdDev, double***** h, double***** tx, double** rx, double* drift, double***** H, int* packet_num, int** packet_time, double* txFilter, double***ZC_sequence, double*** h_preamble)
{
	...
	int preamble_len = PREABLE * (PREABLE_LEN + CP_LEN + CS_LEN);
	...
	for (int nuser = 0; nuser < NUM_USER; nuser++)
	{
		for (int t = 0; t < packet_num[nuser]; t++)
		{
			...
			for (int j = 0; j < PREABLE_LEN; j++)
			{
				pre_packet[(j + CP_LEN) * Unit][0] = ZC_sequence[0][j][0];
				pre_packet[(j + CP_LEN) * Unit][1] = ZC_sequence[0][j][1];
			}
			for (int j = 0; j < CP_LEN; j++)
			{
				pre_packet[j * Unit][0] = pre_packet[(PREABLE_LEN + j) * Unit][0];
				pre_packet[j * Unit][1] = pre_packet[(PREABLE_LEN + j) * Unit][1];
			}
			for (int j = 0; j < CS_LEN; j++)
			{
				pre_packet[((CP_LEN + PREABLE_LEN) + j) * Unit][0] = ZC_sequence[0][j][0];
				pre_packet[((CP_LEN + PREABLE_LEN) + j) * Unit][1] = ZC_sequence[0][j][1];
			}
			for (int i = 0; i < FFT_SEGMENT + DIFF_ENC; i++)
			{
				for (int j = 0; j < effLen; j++)
				{
					pre_packet[(j + i * effLen + preamble_len) * Unit][0] = tx[t][nuser][i][j][0];
					pre_packet[(j + i * effLen + preamble_len) * Unit][1] = tx[t][nuser][i][j][1];
				}
			}
