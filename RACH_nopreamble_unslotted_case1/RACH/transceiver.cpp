...
{
	int ofdmsymbolLen = OVER_SAMPLE ? (CP_LEN + CS_LEN + FFT_POINT) * OVER_SAMPLE_RATE : (CP_LEN + CS_LEN + FFT_POINT);
			...
			*/
			...
			for (int i = 0; i < FFT_SEGMENT + DIFF_ENC; i++)
			{
				for (int j = 0; j < effLen; j++)
				{
					...
					ClassifyRx[t][nuser][i][j][0] = rx[(packet_time[nuser][t]/(CP_LEN+CS_LEN+FFT_POINT))*(CP_LEN+CS_LEN+FFT_POINT) + (j + i * effLen + (PREABLE_LEN + CP_LEN + CS_LEN) * PREABLE) * Unit][0];
					ClassifyRx[t][nuser][i][j][1] = rx[(packet_time[nuser][t]/(CP_LEN+CS_LEN+FFT_POINT))*(CP_LEN+CS_LEN+FFT_POINT) + (j + i * effLen + (PREABLE_LEN + CP_LEN + CS_LEN) * PREABLE) * Unit][1];
