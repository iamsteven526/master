...
void fourwayMLDT(double variance, double*** chCoef, double*** chCoef2,double*** rx, double** app, double** appLlr, double*** finalestimate, double*** finalestimate2, int scma_matrix[SCMA_SOURCE][SCMA_SOURCE * NUM_USER / SCMA_USER_SOURCE], int coef_idx[SCMA_SOURCE][SCMA_SOURCE * NUM_USER / SCMA_USER_SOURCE])
{
	...
	double** chCoefman2 = new double* [NUM_USER];
	for (int nuser = 0; nuser < NUM_USER; ++nuser)
	{
		chCoefman[nuser] = new double[5];
		chCoefman2[nuser] = new double[5];
	}
		...
		for (int source_id = 0; source_id < SCMA_SOURCE; ++source_id) {
			for (int nuser = 0; nuser < NUM_USER * SCMA_SOURCE / SCMA_USER_SOURCE; nuser++)
			{
				if (scma_matrix[source_id][nuser] == 1) {
					nresource = coef_idx[source_id][nuser];
					if (CE_METHOD == 0){
					    chCoefman[count] = finalestimate[nresource][nuser];
					    chCoefman2[count] = finalestimate2[nresource][nuser];
					}
					else{
					    chCoefman[count] = chCoef[nresource][nuser];
					    chCoefman2[count] = chCoef2[nresource][nuser];
					}
					++count;
				}
			}
			...
			for (int appcounts = 0; appcounts < pow(2, (Qm * NUM_USER)); ++appcounts)
			{
				for (int userid = 0; userid < NUM_USER; ++userid)
				{
					for (int qmlevel = 0; qmlevel < Qm; ++qmlevel)
					{
						...
						if (qmlevel == 0) chCoefreg += pow(-1, test + 1) * pow(-1, appcounts / int(pow(2, test - 1))) * chCoef[userid][qmlevel];
						if (qmlevel == 1) chCoefreg += pow(-1, test + 1) * pow(-1, appcounts / int(pow(2, test - 1))) * chCoef2[userid][qmlevel];
					}
				}
			...
			for (int appcounts = 0; appcounts < pow(2, (Qm * NUM_USER)); ++appcounts)
			{
				for (int userid = 0; userid < NUM_USER; ++userid)
				{
					for (int qmlevel = 0; qmlevel < Qm; ++qmlevel)
					{
						...
						if (qmlevel == 0) chCoefreg += (-1) * pow(-1, appcounts / int(pow(2, test - 1))) * chCoef2[userid][qmlevel];
						if (qmlevel == 1) chCoefreg += (-1) * pow(-1, appcounts / int(pow(2, test - 1))) * chCoef[userid][qmlevel];
					}
				}
