...
void Encoder(LDPC &ldpc, PolarCode &polar,int **data, int **codeword, int **Interleaver)
{
	...
	else
	{
		...
		*/
		...
		for (int i = 0, m = 0; i < FFT_POINT*MOD_LEVEL; i++)  //FFT_point = 16
		{
			for (int j = 0; j < FFT_SEGMENT; j++)
			{
				...
				Interleaver[0][m] = (FFT_POINT*MOD_LEVEL)*(m/(FFT_POINT*MOD_LEVEL)) + ((23*m)%(FFT_POINT*MOD_LEVEL));
...
void Modulator(int **codeword, double ****chip, vector<vector<double>> map_16QAM)
{
	for (int nuser = 0; nuser < NUM_USER; nuser++)
	{
		for (int i = 0, m = 0; i < FFT_SEGMENT; i++)
		{
			...
			{
				int index = 8*codeword[nuser][m++] + 4*codeword[nuser][m++] + 2*codeword[nuser][m++] + codeword[nuser][m++];
				chip[nuser][i][0][j] = map_16QAM[index][0];
				chip[nuser][i][1][j] = map_16QAM[index][1];
			}
		}
	...
	if (!OVER_SAMPLE)
	{
		...
		for (int nuser = 0; nuser < NUM_USER; nuser++)
		{
			for (int i = 0; i < FFT_SEGMENT + DIFF_ENC; i++)
			{
				...
				FFT(chip[nuser][i][0], chip[nuser][i][1], FFT_POINT, FFT_LAYER, real, imag, 1, 0); // IFFT
