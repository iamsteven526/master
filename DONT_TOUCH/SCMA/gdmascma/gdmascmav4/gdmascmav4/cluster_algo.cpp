...
void CoefEstimation(double** centroid, double** estimate, double** sample, double** chCoef)
{
	...
	else if (NUM_USER == 2)
	{
		...
		for (int i = 0; i < GROUP_SIZE - 1; i++) // compute the sum of each pair //sum 總共有7+6+5+4+3+2+1
		{
		...
		while (count < (GROUP_SIZE >> 1)) // pairing
		{
			...
			for (int i = 0; i < GROUP_SIZE - 1; i++)
			{
				for (int j = 0; j < GROUP_SIZE - i - 1; j++)
				{
					if (sum[i][j] < min && flag[i] == 0 && flag[i + j + 1] == 0) //若sum小於min則為一個pair
					{
						min = sum[i][j];
						pair[count][0] = i;
						pair[count][1] = i + j + 1;
...
bool ConditionCheck(double** centroid, int* groupSize, int& count)//true 繼續做, false 停下來
{
	for (int i = 0; i < GROUP_SIZE; i++)//檢查GROUP_SIZE個centroid之中點
	{
...
void Generate_square_sequence(vector<vector<double>> & centroid_per, int ** pair, double **centroid,int pair_size,int sequence_num)
{
	...
	vector<vector<int>> sorted_diff_1th; // the number of 等腰三角形
	vector<vector<int>> sorted_diff_2th; // the number of 正方形
	vector<vector<double>> temp_centroid_per;

	while (sorted_diff_2th.size() < sequence_num)  // check第一個等腰三角形
	{
		bound += 0.01;
	...
	/*	for (int i = 0; i < sorted_diff_1th.size(); i++)
		...
		for (int i = 0; i < sorted_diff_1th.size(); i++) // check第二個等腰三角形
		{
	...
	bound = 0.001;
	int count = 0;
	while (temp_centroid_per.size() < sequence_num)//轉換正方形編號 and check正方形
	{
		...
		for (int i = 0; i < sorted_diff_2th.size(); i++)
		{
			...
			vector<double> reg2; // square 上相對的編號
