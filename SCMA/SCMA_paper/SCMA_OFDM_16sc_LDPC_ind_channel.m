...
ldpcDecoder.MaximumIterationCount = 100;
...
for FRAME_CNT = 1:NUM_FRAMES
    ...
    multiblock = 8;
    channelcount = 256/multiblock;
    for pp = 1:V
        for qq = 1:multiblock
            FADE_CHAN(qq,pp,:) = normrnd(0,FADE_STD_DEV,1,CHAN_LEN)+1i*normrnd(0,FADE_STD_DEV,1,CHAN_LEN);
            normal_power = sqrt(0.5*[0.6364, 0.2341, 0.0861, 0.03168, 0.01165]);
            normal_power = reshape(normal_power,[1,1,CHAN_LEN]);
            FADE_CHAN(qq,pp,:) = FADE_CHAN(qq,pp,:).*normal_power;
            FREQ_RESP(qq,pp,:) = fft(FADE_CHAN(qq,pp,:),16);           
        end
        ...
        for symnum = 1:256
            ...
            T_REC_SIG(symnum,:) = T_REC_SIG(symnum,:) + conv(T_SIG_CP(pp,:),reshape(FADE_CHAN(floor((symnum-1)/channelcount+1),pp,:),[1,CHAN_LEN]));
        end      
    end
    ...
    for kkk = 1:K%resource
        for vvv = 1:V%user
            for sc = 1:4
                for qq = 1:multiblock
                    h(kkk,vvv,(sc-1)*256+1+(qq-1)*channelcount:(sc-1)*256+channelcount+(qq-1)*channelcount) = FREQ_RESP(qq,vvv,4*(kkk-1)+sc);
                end
            end
        end
    end
