...
void CoefEstimation(double** centroid, double** estimate, double variance, bool& reset) // TODO reduce complexity
{
	...
	for (int nuser = 0; nuser < NUM_USER*Qm; nuser++)  // *Qm because this file is for SCMA-bpsk
	{
...
void SCMA_MSEComparison(double*** chCoef,double*** chCoef2,double** estimate,double*** finalestimate,double*** finalestimate2,int a[SCMA_SOURCE * NUM_USER / SCMA_USER_SOURCE], int b[SCMA_SOURCE * NUM_USER / SCMA_USER_SOURCE])
{
	...
	for(int i=0; i<SCMA_SOURCE * NUM_USER / SCMA_USER_SOURCE; ++i){
		...
		nresource = b[i];
		tmin = abs(chCoef[nresource][i][0]-finalestimate[nresource][i][0]) + abs(chCoef[nresource][i][1]-finalestimate[nresource][i][1]);
		for(int j=0; j<NUM_USER*Qm; ++j){
            treg = abs(chCoef[nresource][i][0]-1.4142*estimate[j][0]) + abs(chCoef[nresource][i][1]-1.4142*estimate[j][1]);//find closest
			treg2 = abs(chCoef[nresource][i][0]+1.4142*estimate[j][0]) + abs(chCoef[nresource][i][1]+1.4142*estimate[j][1]);//find closest
			if(treg <= tmin){
				tmin = treg;
				finalestimate[nresource][i][0] = 1.4142*estimate[j][0];
				finalestimate[nresource][i][1] = 1.4142*estimate[j][1];
			}
			if(treg2 <= tmin){
				tmin = treg2;
				finalestimate[nresource][i][0] = -1.4142*estimate[j][0];
	...
	for(int i=0; i<SCMA_SOURCE * NUM_USER / SCMA_USER_SOURCE; ++i){
		...
		for(int j=0; j<NUM_USER*Qm; ++j){
			...
			if(treg <= tmin){
				tmin = treg;
				finalestimate2[nresource][i][0] = 1.4142*estimate[j][1];
				finalestimate2[nresource][i][1] = -1.4142*estimate[j][0];
			}
			if(treg2 <= tmin){
				tmin = treg2;
				finalestimate2[nresource][i][0] = -1.4142*estimate[j][1];
