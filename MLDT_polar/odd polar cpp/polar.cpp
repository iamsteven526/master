...
void PolarCode::decode_jpscl_llr(uint16_t list_size, double** app, std::vector<std::vector<int>>& decodedResult)
{
	if (NUM_USER == 1)
	{
...
void PolarCode::decode_BP_Joint(double** app, std::vector<std::vector<int>>& decodedResult)
{
...
std::vector<uint8_t> PolarCode::decode_BP_sep(std::vector<double> & llr, int nuser)
{
	...
	for (int i = 0; i < CODE_LEN; i++)
	{
		if (_frozen_bits[nuser][i])
			continue;
		message_output.push_back(HARD(L[BCT_LAYER][i] + R[BCT_LAYER][i]));
	}
