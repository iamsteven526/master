		...
		for (int m1 = 0; m1 < M; ++m1)
		{
			for (int m2 = 0; m2 < M; ++m2)
			{
				for (int m3 = 0; m3 < M; ++m3)
				{
					...
					{
						f[k][m1][m2][m3][m4] = log(app[k][64 * m1 + 16 * m2 + 4 * m3 + m4]);//TODO
					}
				}
			}
		}
	}
	int ind_df[SCMA_SOURCE][NUM_USER] = { {1,2,4,6}, {0,2,5,7}, {1,3,5,8}, {0,3,4,9} ,{6,7,8,9} };
	int ind_dv[SCMA_SOURCE * NUM_USER / SCMA_USER_SOURCE][SCMA_USER_SOURCE] = { {1,3}, {0,2}, {0,1}, {2,3}, {0,3}, {1,2}, {0,4}, {1,4},{2,4},{3,4} };
			...
			for (int m1 = 0; m1 < M; ++m1)
			{
				...
				for (int m2 = 0; m2 < M; ++m2)
				{
					for (int m3 = 0; m3 < M; ++m3)
					{
						for (int m4 = 0; m4 < M; ++m4)
						{
							sIgv[m2 * M * M + m3 * M + m4] = f[k][m1][m2][m3][m4] + Ivg[k][ind_df[k][1]][m2] + Ivg[k][ind_df[k][2]][m3] + Ivg[k][ind_df[k][3]][m4];
						}
					}
				}
				Igv[k][ind_df[k][0]][m1] = log_sum_exp(sIgv, M * M * M);
			}
			...
			for (int m2 = 0; m2 < M; m2++)
			{
				...
				for (int m1 = 0; m1 < M; m1++)
				{
					for (int m3 = 0; m3 < M; m3++)
					{
						for (int m4 = 0; m4 < M; ++m4)
						{
							sIgv[m1 * M * M + m3 * M + m4] = f[k][m1][m2][m3][m4] + Ivg[k][ind_df[k][0]][m1] + Ivg[k][ind_df[k][2]][m3] + Ivg[k][ind_df[k][3]][m4];
						}
					}
				}
				Igv[k][ind_df[k][1]][m2] = log_sum_exp(sIgv, M * M * M);
			}
			...
			for (int m3 = 0; m3 < M; m3++)
			{
				...
				for (int m1 = 0; m1 < M; m1++)
				{
					for (int m2 = 0; m2 < M; m2++)
					{
						for (int m4 = 0; m4 < M; ++m4)
						{
							sIgv[m1 * M * M + m2 * M + m4] = f[k][m1][m2][m3][m4] + Ivg[k][ind_df[k][0]][m1] + Ivg[k][ind_df[k][1]][m2] + Ivg[k][ind_df[k][3]][m4];
						}
					}
				}
				Igv[k][ind_df[k][2]][m3] = log_sum_exp(sIgv, M * M * M);
			}
			...
			for (int m4 = 0; m4 < M; m4++)
			{
				...
				for (int m1 = 0; m1 < M; m1++)
				{
					for (int m2 = 0; m2 < M; m2++)
					{
						for (int m3 = 0; m3 < M; ++m3)
						{
							sIgv[m1 * M * M + m2 * M + m3] = f[k][m1][m2][m3][m4] + Ivg[k][ind_df[k][0]][m1] + Ivg[k][ind_df[k][1]][m2] + Ivg[k][ind_df[k][3]][m3];
						}
					}
				}
				Igv[k][ind_df[k][3]][m4] = log_sum_exp(sIgv, M * M * M);
			}
		}
