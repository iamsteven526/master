...
{
	if (!OVER_SAMPLE)	 //----------------不做over sampling，只須考率 pulse shaping + CP or CS 
	{
			...
			for (int t = 0; t < packet_num[nuser]; t++)
			{
				for (int i = 0; i < FFT_SEGMENT + DIFF_ENC; i++)
				{
					if (CP_TYPE)			 //---------------- pulse shaping + CP
					{
						...
						{
							preTx[t][nuser][i][j][0] = preTx[t][nuser][i][j][1] = 0;
						}
						for (int j = 0; j < CP_LEN + FFT_POINT; j++)
						{
							preTx[t][nuser][i][j * UP_RATE][0] = tx[t][nuser][i][j][0];
							preTx[t][nuser][i][j * UP_RATE][1] = tx[t][nuser][i][j][1];
						}
					}
				...
				for (int t = 0; t < packet_num[nuser]; t++)
				{
					for (int i = 0; i < FFT_SEGMENT + DIFF_ENC; i++)
					{
						if (CP_TYPE)
						{
							for (int j = 0; j < (CP_LEN + FFT_POINT) * OVER_SAMPLE_RATE; j++)
							{
								preTx[t][nuser][i][j][0] = tx[t][nuser][i][j][0];
								preTx[t][nuser][i][j][1] = tx[t][nuser][i][j][1];
							}
						}
						else
						{
		...
		for (int t = 0; t < packet_num[nuser]; t++)
		{
			for (int i = 0; i < FFT_SEGMENT + DIFF_ENC; i++)
			{
				for (int j = 0; j < effLen; j++) // transmitting filter
				{
					...
					{
						tx[t][nuser][i][j][0] += txFilter[k + trunLen] * preTx[t][nuser][i][(effLen + (j - k) % effLen) % effLen][0]; // real
						tx[t][nuser][i][j][1] += txFilter[k + trunLen] * preTx[t][nuser][i][(effLen + (j - k) % effLen) % effLen][1]; // imaginary
					}
				}
			}
		}
...
void ReceivingFilter(double **rx, double **tempRx, double *rxFilter, int *packet_num, int **packet_time)
{
	...
	for (int i = 0; i < FFT_SEGMENT + DIFF_ENC; i++)
	{
		...
		{
			tempRx[j][0] = tempRx[j][1] = 0;                                  ,mmmmmmmmmmmmmmmmmm
			for (int k = -trunLen; k < trunLen + UP_RATE; k++)
			{
