...
% Step 3: LLR calculation
    ...
    for k = 1:V
        LLR_tmp(3*k-2) = log((exp(Q(1,k))+exp(Q(2,k))+exp(Q(3,k))+exp(Q(4,k)))/(exp(Q(5,k))+exp(Q(6,k))+exp(Q(7,k))+exp(Q(8,k))));
        LLR_tmp(3*k-1) = log((exp(Q(1,k))+exp(Q(2,k))+exp(Q(5,k))+exp(Q(6,k)))/(exp(Q(7,k))+exp(Q(8,k))+exp(Q(3,k))+exp(Q(4,k))));
        LLR_tmp(3*k)   = log((exp(Q(1,k))+exp(Q(3,k))+exp(Q(5,k))+exp(Q(7,k)))/(exp(Q(2,k))+exp(Q(4,k))+exp(Q(6,k))+exp(Q(8,k))));
    end
    LLR(:,jj) = LLR_tmp;
end
