...
public:
	...
	void decode_jpscl_llr(uint16_t list_size, double **app, std::vector<std::vector<int>>& decodedResult, int ** Interleacer); // JPSCL
	void decode_BP_Joint(double **app, std::vector<std::vector<int>>& detectedResult, int **Interleacer);
