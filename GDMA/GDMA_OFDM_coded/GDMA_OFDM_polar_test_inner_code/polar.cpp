...
void PolarCode::encode(int *info_bits, int* codeword, int nuser)
{
	...
	for (uint16_t i = _crc_size; i < _info_length + _crc_size; ++i)
	{
		info_bits_padded.at(_channel_order_sorted.at(nuser).at(i)) = info_bits[i - _crc_size];
	}
	for (uint16_t i = 0; i < 0 + _crc_size; ++i)
	{
		uint8_t crc_bit = 0;
		for (uint16_t j = 0; j < _info_length; ++j)
		{
			crc_bit = (uint8_t)((crc_bit + _crc_matrix.at(i - 0).at(j) * info_bits[j]) % 2);
		}
...
bool PolarCode::crc_check(uint8_t *info_bit_padded, int nuser)
{
	...
	for (int n = 0; n < NUM_USER; n++)
	{
		...
		{
			uint8_t crc_bit = 0;
			for (uint16_t j = _crc_size; j < _crc_size + _info_length; ++j)
			{
				crc_bit = (uint8_t)((crc_bit + _crc_matrix.at(i - 0).at(j - _crc_size) * info_bit_binary.at(n).at(j)) % 2);
			}
...
std::vector<int> PolarCode::decode_scl(int nuser)
{
	...
	for (uint16_t beta = _crc_size; beta < _info_length + _crc_size; ++beta)
	{
		deocded_info_bits[beta - _crc_size] = int(c_0[_channel_order_sorted.at(nuser).at(beta)]);
	}
	for (uint16_t s = 0; s < _list_size; ++s)
	{
		delete[] _arrayPointer_Info.at(s);
