...
%     F_SIG_NO_CP(6,:) = [zeros(1,NUM_BIT) F_SIG_NO_CP_OLD(6,:) F_SIG_NO_CP_OLD(6,:) zeros(1,NUM_BIT)];
    ...
    % IFFT
    T_REC_SIG = zeros(256,8+CP_LEN+CHAN_LEN-1);
    for pp = 1:V
        ...
        FREQ_RESP(pp,:) = fft(FADE_CHAN(pp,:),8);
        for symnum = 1:256
            T_SIG_NO_CP(pp,:) = sqrt(8)*ifft(F_SIG_NO_CP(pp,symnum:256:2048));
    ...
    % AWGN
    AWGN = normrnd(0,1,256,8+CP_LEN+CHAN_LEN-1)+1i*normrnd(0,1,256,8+CP_LEN+CHAN_LEN-1);
    % CHANNEL OUTPUT
    ...
    T_REC_SIG(:,1:CP_LEN) = [];
    for symnum = 1:256
        T_REC_SIG_NO_CP(symnum,:) = T_REC_SIG(symnum,1:8);
        % PERFORMING THE FFT
        F_REC_SIG_NO_CP(symnum,:) = fft(T_REC_SIG_NO_CP(symnum,:))/sqrt(8);
    end
    ...
    for vvv = 1:V
        for sc = 1:8
            h(vvv,(sc-1)*256+1:256*sc) = FREQ_RESP(vvv,sc);
        end
    end
    y = F_REC_SIG_NO_CP_P;
