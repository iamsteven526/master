...
int main()
{
	...
	double **app = new double *[(CODE_LEN + DIFF_ENC*FFT_POINT)/MOD_LEVEL];
	for (int i = 0; i < (CODE_LEN + DIFF_ENC*FFT_POINT)/MOD_LEVEL; i++)
	{
	...
	for (int i = 0; i < NUM_USER; i++)
	{
		...
		for (int j = 0; j < FFT_SEGMENT + DIFF_ENC; j++)
		{
			chip[i][j] = new double*[2];
			for(int k = 0; k < 2; k++){
				chip[i][j][k] = new double[FFT_POINT]; // real and immaginary
			}
		}
	}
	...
	for (int i = 0; i < SNR_NUM; i++)
	{
		...
		double stdDev = sqrt((0.125 / snr) * ((double)(CODE_LEN + DIFF_ENC*FFT_POINT) / (DATA_LEN - (NBC * (FFT_POINT-1)))) * ((double)(CP_LEN + FFT_POINT) / FFT_POINT));
		...
		for (int block = 1; block <= BLOCK_NUM; block++)
		{
			...
			Modulator(codeword, chip, map_16QAM);
			...
			MLDT(ldpc, pow(stdDev, 2), H, postRx, app, appLlr, refLlr, estimate, map_16QAM);
