...
void Clustering(double **rx, double **centroid, int **group, int *groupSize, double *distList, double *variation, double **softAssign, double variance, double **estimate, long double &itCount, double **chCoef, int *known_drift)
{
	...
	while (true)
	{
		...
		{
			if (PROPOSAL == 0)
				InitialSeeding(rx, centroid, group, groupSize, distList, variance, NUM_USER*NUM_TX, GROUP_SIZE, BLOCK_LEN);
			else
				InitialSeeding(rx, centroid, group, groupSize, distList, variance, NUM_USER*NUM_TX, GROUP_SIZE, 2 * BLOCK_LEN);
			reset = false;
		}

		if (PROPOSAL == 0)
			Grouping(rx, centroid, group, groupSize, NUM_USER*NUM_TX, GROUP_SIZE, BLOCK_LEN);
		else
			Grouping(rx, centroid, group, groupSize, NUM_USER*NUM_TX, GROUP_SIZE, 2 * BLOCK_LEN);
...
void MSEComparison(double **chCoef, double **estimate, double &mse,double **rx,double **centroid, int*known_drift)
{
	...
	double temp[NUM_USER*NUM_TX][2] = { 0 };
	...
	for (int i = 0; i < NUM_USER*NUM_TX; i++)
	{
		...
		min_value = 1000;
		min_tr = 0;
		for (int j = 1; j <= NUM_USER*NUM_TX; j++)
		{
			if (map.count(j))
				continue;
...
void printdata(double** rx, double** es, double** chCoef, double** centroid)
{
	...
	for (int j = 0; j < NUM_USER*NUM_TX; j++)
	{
