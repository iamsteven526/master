...
void InitialSeeding(double ***postRx, double **centroid, int **group, int *groupSize, double *distList, int sch, int offset)
{
	...
	{
		//uniform_int_distribution<int> ranPick(offset, (SLIDING ? offset + WINDOW_SIZE : FFT_SEGMENT + DIFF_ENC) - 1);
		//cout << offset << endl;
		//system("pause");
		
		//uniform_int_distribution<int> ranPick(offset,  offset + WINDOW_SIZE - 1);
		int iniPick = offset + int(rand()%WINDOW_SIZE);
		
		//int iniPick = offset +2;
