...
for FRAME_CNT = 1:NUM_FRAMES
    ...
    for pp = 1:V
       for tt = 1:8
           enc(pp,(tt-1)*128 + 1:tt*128) = nrRateMatchPolar(A(pp,(tt-1)*64 + 1:tt*64)',mes,128);
       end
...
%     F_SIG_NO_CP(6,:) = [zeros(1,NUM_BIT) F_SIG_NO_CP_OLD(6,:) F_SIG_NO_CP_OLD(6,:) zeros(1,NUM_BIT)];
    ...
    % IFFT
    T_REC_SIG = zeros(64,16+CP_LEN+CHAN_LEN-1);
    for pp = 1:V
        ...
        for symnum = 1:64
            T_SIG_NO_CP(pp,:) = sqrt(16)*ifft(F_SIG_NO_CP(pp,symnum:64:1024));
    ...
    % AWGN
    AWGN = normrnd(0,1,64,16+CP_LEN+CHAN_LEN-1)+1i*normrnd(0,1,64,16+CP_LEN+CHAN_LEN-1);
    % CHANNEL OUTPUT
...
%     
    LLR = scmadecGDMA2user(y, h, NOISE_STD_DEV*NOISE_STD_DEV);
    LLR(LLR==inf) = 1500;
    LLR(LLR==-inf) = -1500;   
    LLR = LLR*(-1);
