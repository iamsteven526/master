...
std::vector<uint8_t> PolarCode::encode(std::vector<uint8_t> info_bits, int nuser)
{
	...
	{
		info_bits_padded.at(_channel_order_sorted.at(nuser).at(i)) = info_bits.at(i);
	}
	...
	for (uint16_t i = 0; i < _block_length; ++i)
	{
		...
		//std::cout << int(coded_bits.at(i));
	}

	return coded_bits;
}

std::vector<uint8_t> PolarCode::encode_bp(std::vector<uint8_t> info_bits, int nuser)
{
...
std::vector<uint8_t> PolarCode::decode_scl_p1(std::vector<double> p1, std::vector<double> p0, uint16_t list_size, int nuser)
{
...
std::vector<uint8_t> PolarCode::decode_scl_llr(uint16_t list_size, std::vector<double> &llr, int nuser)
{
	for (int i = 0; i < _block_length; i++)
	{
		if (isinf(llr[i]))
...
void PolarCode::decode_jpscl_llr(uint16_t list_size, double** app, std::vector<std::vector<uint8_t>>& decodedResult)
{
	if (NUM_USER == 1)
	{
	...
	/*for (int nuser = 0; nuser < NUM_USER; nuser++)
	{
		...
		for (uint16_t phi = 0; phi < _block_length; ++phi)
		{
			...
			{
				continuePaths_FrozenBit(phi, nuser);
			}
			else
			{
				continuePaths_UnfrozenBit(phi, nuser);
			}
			if ((phi % 2) == 1)
			{
		...
		l = findMostProbablePath((bool)_crc_size, nuser);
		...
		{
			//deocded_info_bits.at(beta) = c_0[_channel_order_sorted.at(nuser).at(beta)];
			decodedResult[nuser][beta] = c_0[_channel_order_sorted.at(nuser).at(beta)];
		}
...
std::vector<uint8_t> PolarCode::decode_scl(int nuser)
{
	for (uint16_t phi = 0; phi < _block_length; ++phi)
	{
		if (_llr_based_computation)
	...
	for (uint16_t l = 0; l < _list_size; ++l)
	{
		...
		if (!JOINT)
		{
			...
			for (uint16_t beta = 0; beta < (1 << (_n - lambda)); ++beta)
			{
				...
				//std::cout << int(c_lambda[2 * beta + 1]) << " ";
			}
		//	system("pause");
	...
	/*for (uint16_t l = 0; l < _list_size; ++l)
	{
		...
		else//---- JPSCL
		{
			...
			if (_llr_based_computation)
			{
				...
				if (app_sum[0] <= NUMERIC_LIMIT) D_N = -LLR_LIMIT;
				else if (app_sum[1] <= NUMERIC_LIMIT) D_N = LLR_LIMIT;
				else D_N = log(app_sum[0] / app_sum[1]);
				
				//---- path metric values for l-th path
				_pathMetric_LLR.at(l) += log(1 + exp(-D_N));
			}
		}
...
void PolarCode::continuePaths_UnfrozenBit(uint16_t phi) 
{
		...
		else
		{
			if (_llr_based_computation)
			{
				...
				else //---- JPSCL
				{
					...
					//---- path forks 
					probForks.at(2 * l) = -(_pathMetric_LLR.at(l) + log(1 + exp(-D_N)));
					probForks.at(2 * l + 1) = -(_pathMetric_LLR.at(l) + log(1 + exp(D_N)));
	...
	for (unsigned l = 0; l < _list_size; ++l)
	{
		...
		else  //---- JPSCL
		{
			...
			if (contForks.at(2 * l) == 1 && contForks.at(2 * l + 1) == 1)
			{	
				//---- set u -> U according to user, and u = 0
				int range = NUM_LEVEL / pow(2, nuser + 1);
				for (int j = 0; j < NUM_LEVEL; j++)
				{
					if (j % (2 * range) < range)
				...
				//---- set u -> U according to user, and u = 1
				range = NUM_LEVEL / pow(2, nuser + 1);
				for (int j = 0; j < NUM_LEVEL; j++)
				{
					if (j % (2 * range) < range)
						c_m[j][(phi % 2)] = NUMERIC_LIMIT;
				...
				if (_llr_based_computation)
				{
					...
					pr_vector = getArrayPointer_pr_vector(_n, l_p);
					//---- D_N calculation
					range = NUM_LEVEL / pow(2, nuser + 1);
					app_sum[0] = 0;
					app_sum[1] = 0;
					...
					_pathMetric_LLR.at(l_p) += log(1 + exp(D_N));
			...
			else
			{
				if (contForks.at(2 * l) == 1)
				{
					...
					{
						if (j % (2 * range) < range)
							c_m[j][(phi % 2)] = 1 / (double(NUM_LEVEL) / 2);
						else
							c_m[j][(phi % 2)] = NUMERIC_LIMIT;
					}
...
for (unsigned l = 0; l < _list_size; ++l)
{
	...
	if (!JOINT && contForks.at(2 * l) == 0 && contForks.at(2 * l + 1) == 0) //被淘汰的path
	{
		killPath(l);
	}
	if (JOINT)
...
for (unsigned l = 0; l < _list_size; ++l)
{
	if (!JOINT) //---- SCL
	{
		if (contForks.at(2 * l) == 0 && contForks.at(2 * l + 1) == 0) //兩個child接被淘汰的path
		{
			continue;
		}
		uint8_t* c_m = getArrayPointer_C(_n, l); // 用來紀錄最後一層的 0 1
		if (contForks.at(2 * l) == 1 && contForks.at(2 * l + 1) == 1) //兩個child接沒被淘汰
		{
			c_m[(phi % 2)] = 0;
			uint16_t l_p = clonePath(l);
			c_m = getArrayPointer_C(_n, l_p);
			c_m[(phi % 2)] = 1;
			std::copy(_arrayPointer_Info.at(l), _arrayPointer_Info.at(l) + phi, _arrayPointer_Info.at(l_p)); // _arrayPointer_Info.at[...]為用來記錄第l-pa
			_arrayPointer_Info.at(l)[phi] = 0;
			_arrayPointer_Info.at(l_p)[phi] = 1;
			if (_llr_based_computation)
			{
		...
		{
			if (contForks.at(2 * l) == 1)// 第一個child被淘汰，也就是bit=1被淘汰
			{
				c_m[(phi % 2)] = 0;
				_arrayPointer_Info.at(l)[phi] = 0;
				if (_llr_based_computation)
	...
	else  //---- JPSCL
	{
		...
		double** c_m = getArrayPointer_C_vector(_n, l_p); // 用來紀錄最後一層的 疊加訊號機率
		for (int k = 0; k < fork_size; k++)
		{
			if (contForks.at(fork_size * l + k) == 1) //判斷4個child是否符合資格
			{
				if (tr == 1) //若這條path前面有人用過 造新的一條
				{
					l_p = clonePath(l); //造新的
					_pathMetric_LLR.at(l_p) = temp;
					c_m = getArrayPointer_C_vector(_n, l_p);
					std::copy(_arrayPointer_Info.at(l), _arrayPointer_Info.at(l) + phi, _arrayPointer_Info.at(l_p));//複製同一條資料前面的訊息
				}
...
void PolarCode::decode_BP_Joint(double** app, std::vector<std::vector<uint8_t>>& decodedResult)
{
...
std::vector<uint8_t> PolarCode::decode_BP_sep(std::vector<double> & llr, int nuser)
{
	for (int i = 0; i < _block_length; i++)
	{
		if (isinf(llr[i]))
		...
		for (int i = 0; i < BCT_LAYER; i++)
		{
			...
			for (int g = 0; g < G; g++)
			{
				...
				for (int j = 0; j < CODE_LEN / p; j++)
				{
					...
					//L[i + 1][upper] = L[i + 1][upper] < -40 ? -40 : L[i + 1][upper] > 40 ? 40 : L[i + 1][upper];
					//L[i + 1][lower] = L[i + 1][upper] < -40 ? -40 : L[i + 1][lower] > 40 ? 40 : L[i + 1][lower];
				}
			}
			...
			for (int i = BCT_LAYER; i >= 0; i--)
			{
				...
				for (int g = 0; g < G; g++)
				{
					...
					for (int j = 0; j < CODE_LEN / p; j++)
					{
						...
						//R[i + 1][upper] = R[i + 1][upper] < -40 ? -40 : R[i + 1][upper] > 40 ? 40 : R[i + 1][upper];
						//R[i + 1][lower] = R[i + 1][upper] < -40 ? -40 : R[i + 1][lower] > 40 ? 40 : R[i + 1][lower];
					}
				}
			}
		}
...
double PolarCode::min_sum(double a, double b)
{
	...
	*/
	...
	/*if (2 * atanh(tanh(a / 2) * tanh(b / 2)) > 30)
		return 40;
	else if (2 * atanh(tanh(a / 2) * tanh(b / 2)) < -30)
		return -40;
	else
