...
int main()
{
	...
	double** chCoef = new double* [NUM_USER * SCMA_SOURCE / SCMA_USER_SOURCE];//BPSK user1
	double** chCoef2 = new double* [NUM_USER * SCMA_SOURCE / SCMA_USER_SOURCE];//BPSK user2
	...
	for (int i = 0; i < SNR_NUM; i++)
	{
		...
		for (int block = 1; block <= BLOCK_NUM; ++block)
		{
			...
			for (int source_id = 0; source_id < SCMA_SOURCE; ++source_id) {//?				
				MultipleAccessChannel(stdDev, chCoef, chCoef2, tx, rx[source_id], source_id, scma_matrix);
			}
			/*
